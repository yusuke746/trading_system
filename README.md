...
Pine Script（5M足）が生成したアラートJSONをVPS上のFlask Webhookで受信し、
数値ルールベースのスコアリングエンジン（LLM不使用）でエントリー可否を判定。
承認されたシグナルをWindows上のMT5に発注する。

```
TradingView (Pine Script 5M)
        │  アラートJSON (Webhook)
        ▼
Flask Webhook Server  ← VPS: http://45.76.51.249:80/webhook
        │
        ▼
scoring_engine.py  ← ゲートチェック → スコア計算 → approve/wait/reject
        │  approve のみ
        ▼
MT5 (Windows PC)  ← XMTrading / GOLD
        │
        ▼
...
### 1. VPS側（Flask Webhook）

```bash
# 依存パッケージのインストール（MT5はLinux非対応のためスキップ）
pip install flask flask-cors requests pandas numpy optuna \
  python-dotenv --break-system-packages --ignore-installed

# バックグラウンド起動
mkdir -p /root/trading_system/logs
nohup bash -c 'PYTHONPATH=/root/trading_system \
  python3 /root/trading_system/python/app.py' \
  > /root/trading_system/logs/app.log 2>&1 &

# 起動確認
curl -s http://localhost:80/health
```

※ MT5はWindowsマシン側で別途起動すること。
※ メインのFlaskエントリーポイントは `python/app.py`。

### 2. TradingView 設定

- Pine Script `pine/alert_sender_5m.pine` をチャートに追加
- アラート設定 → Webhook URL: `http://45.76.51.249:80/webhook`
...
```bash
# サーバー死活確認
curl http://45.76.51.249:80/health

# テスト Webhook 送信
curl -X POST http://45.76.51.249:80/webhook \
