...
├── tests/
│   ├── test_scoring_engine.py  # スコアリングエンジン（40件）
│   ├── test_logger_module.py   # logger_module（11件）
...
| 条件 |
|---|
| なし（Gate3廃止）→ スコアリングで品質管理 |

> v4.8変更理由：113日・438件のCSVデータ分析で、BREAKOUTのGate3（fvg_aligned OR zone_aligned）を適用すると件数が激減（4.6件/日→1.8件/日）し統計的に不十分となることが判明。Gate3廃止し、スコアテーブルで品質管理する方式に変更。
...
- **BOS Pending管理:** BOSバーから `bos_timeout_bars`（デフォルト**12本=60分**、旧20本）以内にプルバックがなければキャンセル
  - 変更根拠：seq別分析で seq=12以降はPF<1.0（seq=1〜11は全てPF>1.0）
...
- `bull_fvg_used[]` / `bear_fvg_used[]` boolean配列で一度タッチしたFVGを使用済みマークし、再タッチでの再発火を防止（**v4.6実装済み**）
...
- `dem_used[]` / `sup_used[]` boolean配列で同様に再タッチ制御を実装（**v4.6実装済み**）
...
Gate2（レジーム確認）
    ↓ 通過
Gate3（SMC条件確認、BREAKOUTはGate3なし）
    ↓ 通過
スコア計算（レジーム別テーブルで加算）
    ↓
...
| NYセッション | `session_ny` | **+0.10** | CSV分析: PF=1.07（旧+0.05から強化） |
| Offアワーズ | `session_off` | **-0.05** | CSV分析: PF=0.93（旧-0.20から緩和） |
| London/NYオーバーラップ | `session_london_ny` | **-0.15** | CSV分析: PF=0.89（旧-0.10から強化） |
| Londonセッション | `session_london` | **-0.25** | CSV分析: PF=0.83（旧-0.15から強化） |
...
| 条件 | キー | 加減点 | 内容 |
|---|---|---|---|
| ブレイク確認（常時） | `breakout_base` | **+0.30** | BREAKOUTレジーム確定時に必ず加算 |
| FVGリテスト | `breakout_fvg_retest` | **+0.15** | ブレイク後にFVGヒット確認 |
| Zoneリテスト | `breakout_zone_retest` | **+0.10** | ブレイク後にZoneヒット確認 |
...
| 高インパクトニュース前後 | `news_nearby` | **-0.30** | TRENDと共通ペナルティ |
...
| bos_alert_seq | 同一BOSイベント内での発火回数（0=BOS無関係） |
| fvg_aligned | FVG方向一致ヒット（再タッチ制御後）true=1, false=0 |
| zone_aligned | Zone方向一致ヒット（再タッチ制御後）true=1, false=0 |
| ob_atr_ratio | ヒットしたOBのサイズ÷ATR5 |
| bos_age_bars | BOS発生から現在バーまでの経過本数 |
| fvg_size_atr | ヒットしたFVGのサイズ÷ATR5 |
| h4_direction | H4足トレンド方向（bull=1, bear=-1） |
| h4_adx | H4足ADX値（トレンド強度） |
| momentum_ratio | 直近5本の値幅 ÷ ATR（モメンタム強度） |
| candle_body_ratio | キャンドル実体÷全体値幅（0〜1、1=全部実体） |
| upper_wick_ratio | 上ヒゲ÷全体値幅（売り圧力の代理変数） |
| lower_wick_ratio | 下ヒゲ÷全体値幅（買い圧力の代理変数） |
| h1_choch | H1足でCHoCH発生（true=1, false=0） |
| h1_ema_distance | closeとH1 EMAの距離÷ATR（乖離度） |

### scoring_history テーブルスキーマ（v4.8追加）

`logger_module.log_scoring_history()` が全アラートをSQLiteに記録する。
v4.8でSMCフラグ列を追加し、事後検証が可能になった。

| カラム | 型 | 内容 |
|---|---|---|
| `id` | INTEGER PK | 自動採番 |
| `created_at` | TEXT | INSERT日時（UTC） |
| `signal_direction` | TEXT | buy / sell |
| `regime` | TEXT | TREND / BREAKOUT |
| `session` | TEXT | london_ny / london / ny / tokyo / off |
| `total_score` | REAL | 合計スコア |
| `decision` | TEXT | approve / wait / reject |
| `breakdown_json` | TEXT | スコア内訳JSON |
| `fvg_aligned` | INTEGER | FVGヒット（0/1） |
| `zone_aligned` | INTEGER | Zoneヒット（0/1） |
| `bos_confirmed` | INTEGER | BOS確認（0/1） |
| `ob_aligned` | INTEGER | OBヒット（0/1） |
| `choch_confirmed` | INTEGER | CHoCH確認（0/1） |
| `outcome` | TEXT | win / loss / breakeven（決済後に更新） |
| `pnl_usd` | REAL | 損益USD（決済後に更新） |

`executions.ai_decision_id` → `scoring_history.id` の外部キーで紐付け。
ポジション決済時に `loss_analyzer.py` が `outcome` と `pnl_usd` を自動更新する（v4.8修正）。
...
### Phase 1 ─ スコアリング精度向上（進行中）

**Go条件:** TRENDレジームの月次PF ≥ 1.20 かつ BREAKOUTレジームの月次PF ≥ 1.10 が3ヶ月連続で確認できたらPhase 2へ。

| タスク | 内容 | 状態 |
|---|---|---|
| Phase 1-1 | BOS/OBバグ修正（CHoCHリセット前にBOS評価する処理順序修正） | ✅ 完了 |
| Phase 1-1（試み） | CHoCH再定義（Lower High条件追加） | ❌ 却下（90日データで月次EV悪化） |
| Phase 1-2（試み） | OB品質フィルター（ATRサイズ条件） | ❌ 却下（OB件数が激減、PF低下） |
| Phase 1-3 | スコアリング全面再設計（CSVデータ根拠）・閾値0.50に変更 | ✅ 完了（v4.4） |
| Phase 1-4 | BREAKOUTレジーム判定をデータドリブン2段階方式に改修 | ✅ 完了（v4.5） |
| Phase 1-5 | FVG/Zone再使用制御（再タッチ制御）実装 | ✅ 完了（v4.6） |
| Phase 1-6 | BREAKOUTエグジット再設計（TP×3.5） | ✅ 完了（v4.6） |
| Phase 1-7 | TREND/BREAKOUTスコアリング完全分離・Bug修正2件 | ✅ 完了（v4.8） |
| Phase 1-8 | csv_exporterにPhase 1分析フィールド追加（h4_direction等8件） | 🔲 未着手 |
| Phase 1-9 | ニュースフィルター動的実装（経済指標カレンダーAPI連携） | 🔲 未着手 |

### Phase 2 ─ 上位時間軸フィルター強化（未着手）

**Go条件:** Phase 1完了 + ライブデータ100件以上蓄積。

| タスク | 内容 | 状態 |
|---|---|---|
| Phase 2-1 | H4足方向フィルター追加（h4_direction / h4_adx を scoring_engine に組み込み） | 🔲 未着手 |
| Phase 2-2 | FVG/Zoneを15M足ベースに変更（5M足のノイズ対策） | 🔲 検討中 |

### Phase 3 ─ エントリー精度向上（未着手）

**Go条件:** Phase 2完了。

| タスク | 内容 | 状態 |
|---|---|---|
| Phase 3-1 | RSIダイバージェンス実装（スコア0.00の項目を有効化） | 🔲 未着手 |
| Phase 3-2 | キャンドル形状フィルター（candle_body_ratio / wick_ratio） | 🔲 未着手 |

### Phase 4 ─ ポジション管理高度化（未着手）

**Go条件:** Phase 3完了 + 月次PF ≥ 1.30 が確認できた時点。

| タスク | 内容 | 状態 |
|---|---|---|
| Phase 4-1 | ピラミッディング（FVG識別子実装後に実施） | 🔲 未着手 |
| Phase 4-2 | トレーリングSL幅最適化（ATR×1.2の最適化・ライブデータ蓄積後） | 🔲 未着手 |

### Phase 5（将来候補）：RANGEレジーム逆張り戦略

**実装タイミング：Phase 1完了後かつTREND PF≥1.30で3ヶ月安定してから**

#### 背景・目的

現行システムはTRENDとBREAKOUTのみを対象とするため、
GOLDの相場時間の6〜8割を占めるレンジ相場では
エントリー機会がゼロになる日が発生する。
レンジ逆張り戦略を独立システムとして追加することで
全相場環境での安定したエントリー頻度を確保する。

#### 設計方針（暫定）

**レンジ認定条件（Stage1）:**
- `adx_avg_24 < 20`（直近24本のADX平均が低い）
- `box_atr_ratio_24 > 2.0 AND < 6.0`（レンジ幅がATRの2〜6倍）

**エントリー条件（Stage2）:**
- レンジ上限から `ATR × 0.3` 以内 → sell
- レンジ下限から `ATR × 0.3` 以内 → buy

**エグジット:**
- TP：レンジ幅の50%（中央値まで）
- SL：レンジ外 `ATR × 0.5`
- RR：約1:1〜1.5:1（TRENDより低いが勝率60〜70%を狙う）

**フィルター:**
- ニュース前後は除外（既存news_filterを流用）
- H1方向との整合性チェック

#### 重要制約

> ⚠️ **既存のTREND/BREAKOUTロジックと混在禁止。**
> 独立したスコアリング・独立したDB記録として並行テストを行う。
> 既存システムのPFやデータを汚染しないことが最優先。
...
|---|---|
| TRENDのPF | ≥1.30（3ヶ月連続） |
| Phase 1完了 | 急反転パターン特定済み |
| 実装方式 | 独立システムとして並行デモテスト |
...
| ✅ 解決済み | scoring_history に SMC フラグ未保存 | v4.8にて `fvg_aligned`/`zone_aligned`/`bos_confirmed`/`ob_aligned`/`choch_confirmed` をDBに記録するよう修正済み |
...
| バグ | 原因 | 修正内容 |
|---|---|---|
| `executions.ai_decision_id` が常にNULL | `log_scoring_history()` が `lastrowid` を返していなかった + `app.py` が `ai_decision_id=None` を渡していた | `log_scoring_history()` で `cursor.lastrowid` を返すよう修正・`app.py` で戻り値を受け取り `scoring_history_id` として渡すよう修正 |
| `scoring_history` の `outcome`/`pnl_usd` が更新されない | `loss_analyzer.py` の `_update_scoring_history()` が `ai_decisions` テーブルへの壊れたJOINを使用していた | `executions.ai_decision_id` を直接参照する方式に書き直し・`update_scoring_history_outcome()` 関数を新設 |
...
| ファイル | テスト数 | 対象 |
|---|---|---|
| `test_scoring_engine.py` | **40件** | Gate1〜3判定・スコア計算・閾値判定・ペナルティ関数・BREAKOUT分離スコアリング |
| `test_logger_module.py` | **11件** | log_scoring_history戻り値・ai_decision_id保存・outcome更新 |
