...
```
TradingView (Pine Script 5M / v4.1)
...
├── scoring_engine.py         # ゲートチェック + スコアリングエンジン（v4.1）
...
| P3.6 | should_alert条件緩和（Gate3重複解消）・Discord通知実装 | ✅ 完了 |
...
| 2 | `news_nearby` | Pine Script で常に `false` 固定。ニュース時刻照合未実装。**⚠️ 重要指標時のエントリーをブロックできない**（スコア −0.30 の安全装置が無効） |
| 3 | スコアリングテーブル | 全レジーム共通採点。レジーム別の加減点設計が未実装 |
| 4 | BREAKOUT direction | `direction="none"` になりうるケースが未ガード（Gate2通過後にnoneが混入する可能性） |
| 5 | バックテストサンプル数 | `alerts_test.csv` は94件/50日分。6ヶ月以上のライブデータ取得が必要 |
