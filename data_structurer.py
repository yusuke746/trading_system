"""
data_structurer.py - ルールベースによる生データの正規化構造変換
AI Trading System v3.5

デフォルトはルールベース（_fallback_structurize）で動作する。
高速・確定的・APIコストゼロ。

実験ルート: 環境変数 LLM_STRUCTURIZE=1 のときのみ LLM（OpenAI）を使用。
失敗時はルールベースにフォールバックする。
"""
...
def _fallback_structurize(context: dict) -> dict:
    ...
    # perfect_order が None でも fields_missing には追加しない（スコア計算未使用）

    # higher_highs / lower_lows は時系列データが必要なため未実装
    # None のまま返すが fields_missing には追加しない
    # （scoring_engine のスコア計算でも未使用のため欠損扱い不要）
