...
class TestSafetyCheck(unittest.TestCase):
    ...
    def test_valid_proposal_passes(self):
        ...
        base = SCORING_CONFIG.get("choch_strong", 0.20)
        ok, reason = self.opt._safety_check({
            "proposals": {"choch_strong": round(base + 0.04, 4)}
        })
    ...
    def test_too_large_change_rejected(self):
        ...
        base = SCORING_CONFIG.get("choch_strong", 0.20)
        ok, reason = self.opt._safety_check({
            "proposals": {"choch_strong": round(base + 0.10, 4)}
        })
        self.assertFalse(ok)
    ...
    def test_threshold_param_stricter_limit(self):
        ...
        from config import SCORING_CONFIG
        base = SCORING_CONFIG.get("approve_threshold", 0.25)
        # ±0.04 は MAX_THRESHOLD_CHANGE=0.03 を超えるため拒否されるべき
        ok, reason = self.opt._safety_check({
    ...
    def test_out_of_range_rejected(self):
        ...
        ok, reason = self.opt._safety_check({
            "proposals": {"fvg_and_zone_overlap": 0.99}  # max=0.30 を大幅超過
        })
        self.assertFalse(ok)
...
class TestApplyConfig(unittest.TestCase):
    ...
    def test_apply_writes_new_value(self):
        ...
        changes = self.opt._apply_config({
            "proposals": {"choch_strong": new_val}
        })

        self.assertIn("choch_strong", changes)
        self.assertAlmostEqual(changes["choch_strong"]["new"], new_val, places=4)
