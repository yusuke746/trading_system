"""
tests/test_data_structurer.py - data_structurer.py のユニットテスト
...
from data_structurer import _fallback_structurize, _validate_and_fix_schema, _safe_float
...
class TestFallbackStructurize(unittest.TestCase):
    ...
    def test_higher_highs_lower_lows_not_in_fields_missing(self):
        """higher_highs / lower_lows / perfect_order は fields_missing に含まれない"""
        ctx = _make_context()
        result = _fallback_structurize(ctx)
        missing = result["data_completeness"]["fields_missing"]
        self.assertNotIn("higher_highs", missing)
        self.assertNotIn("lower_lows", missing)
        self.assertNotIn("perfect_order", missing)

    def test_adx_rising_added_when_adx_none(self):
        """adx_value が None のとき adx_rising も fields_missing に追加される"""
        ctx = _make_context(adx14=None)
        result = _fallback_structurize(ctx)
        missing = result["data_completeness"]["fields_missing"]
        self.assertIn("adx_value", missing)
    ...
    def test_no_instant_reject_with_valid_data(self):
        """全データが揃っているとき fields_missing が3件未満になりrejectされない"""
        ctx = _make_context()  # デフォルトは全データ揃っている
        result = _fallback_structurize(ctx)
        missing = result["data_completeness"]["fields_missing"]
        self.assertLess(len(missing), 3,
            f"fields_missing が3件以上あると即rejectになる: {missing}")
